...
load("~/Disk Google/Repositories/marketing-data-analysis/Yoghurt/yoghurt.RData")
getwd()
...
yoghurt.df$spend <- exp(rnorm(n=yoghurt.df$?..id, mean=(as.integer(yoghurt.df$income)/5), sd=0.5))*1000
yoghurt.df$spend <- round(yoghurt.df$spend, digits=2)
## total spending for yoghurt
yoghurt.df$yogspend <- exp(rnorm(n=yoghurt.df$?..id, mean=log(yoghurt.df$spend*0.1), sd=0.5))/10
